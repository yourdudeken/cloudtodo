/*
  ...
  2. Functions
    - `handle_task_notification()`: Sends email notifications for tasks
    - `notify_task_reminder()`: Trigger function for task reminders
...
BEGIN
  ...
  WHERE user_id = NEW.user_id
    AND type = CASE
      WHEN NEW.reminder IS NOT NULL THEN 'reminder'
  ...
  IF v_notification_enabled THEN
    ...
    WHERE name = CASE
      WHEN NEW.reminder IS NOT NULL THEN 'task_reminder'
          ...
          'html', replace(
            replace(
              replace(
                ...
                '{{due_date}}', to_char(NEW.due_date, 'DD Mon YYYY HH24:MI')
              ),
...
CREATE TRIGGER task_notification_trigger
  AFTER INSERT OR UPDATE OF due_date, reminder
  ON tasks
  FOR EACH ROW
